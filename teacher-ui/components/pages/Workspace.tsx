...
interface WorkspaceData {
  filieres: Filiere[];
  groupsByLevel: Record<number, LevelData>;
}
...
export default function Workspace() {
  ...
  const fetchWorkspace = async (filiereId?: number) => {
    filiereId ? setFiliereLoading(true) : setLoading(true);
    setError(null);

    try {
      const url = filiereId
        ? `/workspace/modules?filiere_id=${filiereId}`
        : `/workspace/modules`;
      ...
      if (!filiereId && data.filieres.length > 0) {
        setSelectedFiliere(data.filieres[0]);
      }
    } catch (err) {
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
                  {levelData.groups.map(group => (
                    <div
                      ...
                      className="flex flex-col px-2 py-1 bg-[#7678ED] rounded-[10px] w-full min-w-[300px] gap-4 hover:shadow-lg transition-shadow cursor-pointer"
                    >
