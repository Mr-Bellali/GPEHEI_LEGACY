...
class WorkspaceController
{
    ...
    public function index()
    {
        ...
        // If it's an AJAX request, only return the workspace content div
        if ($isAjax) {
            ...
            // Extract just the workspace content div
            preg_match('/<div id="workspaceContent".*?>(.*?)<\/div>\s*<div><\/div>/s', $html, $matches);
            preg_match('/<div id="promotionInfo".*?>(.*?)<\/div>/s', $html, $promoMatches);
            
            $response = [
                'content' => $matches[1] ?? $html,
                'promotionInfo' => $promoMatches[1] ?? ''
            ];
        ...
        // Include the full view for regular requests
        include __DIR__ . '/../views/pages/workspace.php';
    }
}
?>